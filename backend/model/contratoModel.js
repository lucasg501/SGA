...
class ContratoModel {
    ...
    constructor(idContrato, idImovel, idLocatario, idLocador, qtdParcelas, valorParcela, dataVencimento, inicioVigenciaContrato, fimVigenciaContrato, multa, juros, ativo) {
    ...
    async listar() {
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                lista.push(new ContratoModel(rows[i]['idContrato'], rows[i]['idImovel'], rows[i]['idLocatario'], rows[i]['idLocador'], rows[i]['qtdParcelas'], rows[i]['valorParcela'], rows[i]['dataVencimento'], rows[i]['inicioVigenciaContrato'], rows[i]['fimVigenciaContrato'], rows[i]['multa'], rows[i]['juros'], rows[i]['ativo']));
            }
            return lista;
        } catch (e) {
            console.log(e);
    ...
    async obter(idContrato) {
        try {
            ...
            if (rows.length > 0) {
                let contrato = new ContratoModel(rows[0]['idContrato'], rows[0]['idImovel'], rows[0]['idLocatario'], rows[0]['idLocador'], rows[0]['qtdParcelas'], rows[0]['valorParcela'], rows[0]['dataVencimento'], rows[0]['inicioVigenciaContrato'], rows[0]['fimVigenciaContrato'], rows[0]['multa'], rows[0]['juros'], rows[0]['ativo']);
                return contrato;
            } else {
                return null;
            }
    ...
    async gravar() {
        try {
            if (this.#idContrato == 0) {
                ...
                let sql = "insert into contrato (idImovel, idLocatario, idLocador, qtdParcelas, valorParcela, dataVencimento, inicioVigenciaContrato, fimVigenciaContrato, multa, juros, ativo) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                let valores = [this.#idImovel, this.#idLocatario, this.#idLocador, this.#qtdParcelas, this.#valorParcela, this.#dataVencimento, this.#inicioVigenciaContrato, this.#fimVigenciaContrato, this.#multa, this.#juros, this.#ativo];
                let ok = await banco.ExecutaComandoNonQuery(sql, valores);
                return ok;
            } else {
                let sql = "update contrato set idImovel = ?, idLocatario = ?, idLocador = ?, qtdParcelas = ?, valorParcela = ?, dataVencimento = ?, inicioVigenciaContrato = ?, fimVigenciaContrato = ?, multa = ?, juros = ?, ativo = ? where idContrato = ?";
                let valores = [this.#idImovel, this.#idLocatario, this.#idLocador, this.#qtdParcelas, this.#valorParcela, this.#dataVencimento, this.#inicioVigenciaContrato, this.#fimVigenciaContrato, this.#multa, this.#juros, this.#ativo,this.#idContrato];
                let ok = await banco.ExecutaComandoNonQuery(sql, valores);
                return ok;
            }
        } catch (e) {
    ...
    async buscaPorImovel(refImovel, limit = 10, offset = 0) {
        try {
            ...
            const sql = "SELECT c.idContrato, c.idImovel, c.idLocatario, c.idLocador, c.qtdParcelas, c.valorParcela, c.dataVencimento, c.inicioVigenciaContrato, c.fimVigenciaContrato, c.multa, c.juros, c.ativo, i.refImovel FROM contrato c JOIN imovel i ON i.idImovel = c.idImovel WHERE i.refImovel LIKE CONCAT('%', ?, '%') ORDER BY c.idContrato DESC LIMIT ? OFFSET ?;";
